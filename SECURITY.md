...
- **APIM validates but does not enforce group membership.** The APIM policy validates that callers have a valid Entra ID JWT with an `oid` claim. It does not check security group membership. Group-based access control is enforced at the Entra ID level (Conditional Access) and at the API Center RBAC level (Data Reader role assigned to the security group).
